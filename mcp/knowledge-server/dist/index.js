...
import { z } from 'zod';
import { listDocs, getDoc, downloadDoc, createDoc } from './client.js';
...
server.registerTool('list_docs', {
    ...
    const result = await listDocs(source, { category, tag, page, pageSize });
...
server.registerTool('search_docs', {
    ...
    const result = await listDocs(source, { q: query, page, pageSize });
    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
});
server.registerTool('get_doc', {
    ...
    const doc = await getDoc(source, id, format);
    return { content: [{ type: 'text', text: JSON.stringify(doc, null, 2) }] };
});
server.registerTool('download_doc', {
    ...
    const text = await downloadDoc(source, id, format);
...
server.registerTool('create_doc', {
    ...
    const result = await createDoc(source, {
        title,
        content,
        category,
        tags: parsedTags,
