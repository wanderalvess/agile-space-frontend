  ...
  const handleConvert = () => {
    ...
    try {
      if (mode === 'YAML_TO_JSON') {
        ...
        setOutput(dumpYaml(parsed, { indent: 2, lineWidth: -1 }));
      }
      setError(null);
      toast({ title: "Conversão concluída!" });
    } catch (e: any) {
      setOutput('');
      const message = mode === 'YAML_TO_JSON'
        ? `YAML inválido: ${e?.reason || e?.message || 'erro de sintaxe desconhecido'}`
        : `JSON inválido: ${e?.message || 'erro de sintaxe desconhecido'}`;
      setError(message);
      toast({ title: "Erro na conversão", description: message, variant: "destructive" });
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-8 gap-6 overflow-hidden">
          ...
          <div className="flex items-center gap-2 bg-white p-1 rounded-2xl border border-slate-100 w-fit shadow-sm">
             <button
               onClick={() => { setMode('YAML_TO_JSON'); setOutput(''); setError(null); }}
             ...
             <button
               onClick={() => { setMode('JSON_TO_YAML'); setOutput(''); setError(null); }}
          ...
          <div className="flex-1 grid grid-cols-2 gap-8 overflow-hidden">
             ...
             <div className="flex flex-col gap-4">
                ...
                <Card className={cn(
                  "flex-1 border-none shadow-inner rounded-[2.5rem] overflow-hidden p-8 relative",
                  error ? "bg-red-950" : "bg-slate-900"
                )}>
                   {error ? (
                      <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-6">
                         <AlertCircle className="h-10 w-10 text-red-400" />
                         <span className="text-[11px] font-bold text-red-200 leading-relaxed">{error}</span>
                      </div>
                   ) : !output ? (
      ...
      <div className="px-8 py-3 bg-white border-t flex items-center gap-3">
         <RefreshCw className="h-4 w-4 text-amber-400 animate-pulse" />
