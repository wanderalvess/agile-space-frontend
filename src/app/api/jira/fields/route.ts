...
export async function POST(req: NextRequest) {
  ...
  if (!checkRateLimit(`jira-fields:${ip}`, 100, 60_000)) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/jira/fields`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Erro ao buscar campos do Jira via Spring Boot', details: errorText },
