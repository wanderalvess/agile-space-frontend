...
export async function POST(req: NextRequest) {
  ...
  if (!checkRateLimit(`jira-sprint:${ip}`, 100, 60_000)) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/jira/sprint`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Erro ao buscar sprint do Jira via Spring Boot', details: errorText },
